...
public class Add_Supplier2 extends Base_Class{
...
public void add_supplier() throws InterruptedException {
	...
	Xls_Reader reader = new Xls_Reader("C:\\Users\\47Billion\\Desktop\\xlsx\\Add_Supplier_Afghanistan.xlsx");
	 ...
	 int count = reader.getRowCount("Add_supplier_Full_Sanity");
	 
	 for(int i=0;i<count-1;i++) {
		
		 User_Invited=reader.getCellData("Add_supplier_Full_Sanity", "User_Invited", i+2);
		 
		 if(User_Invited.equals("No")) {
			 
		 Company_Name = reader.getCellData("Add_supplier_Full_Sanity", "Company_Name", i+2);	 
		 Contact_Email=reader.getCellData("Add_supplier_Full_Sanity", "Contact_Email", i+2); 
		 Contact_name =reader.getCellData("Add_supplier_Full_Sanity", "Contact_name", i+2);
		 Countries_of_operation  =  reader.getCellData("Add_supplier_Full_Sanity", "Countries_of_operation", i+2);
		 Product_or_services = reader.getCellData("Add_supplier_Full_Sanity","Product_or_services", i+2);
         Spend_volume= reader.getCellData("Add_supplier_Full_Sanity", "Spend_volume", i+2);
	...
	reader.setCellData("Add_supplier_Full_Sanity","User_Invited",i+2,"Yes");
	...
	break;
	 ...
	 reader.setCellData("Recent_Added","Incident_title",2,Company_Name);
